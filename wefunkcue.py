...
class Client:
    ...
    def DownloadTorrent(self, filename, output_dir):
        torrent_name = filename + ".torrent"
        url = "http://www.wefunkradio.com/torrent/" + torrent_name
        try:
            req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
            with urllib.request.urlopen(req) as r, open(os.path.join(output_dir, torrent_name), 'wb') as f:
                shutil.copyfileobj(r, f)
            print(f"   [Torrent] Downloaded")
        except Exception:
            pass 
...
def main():
    ...
    for number in range(args.start, args.end + 1):
        ...
        if show_info and filename and html_bytes:
            ...
            client.DownloadTorrent(filename, args.output_dir)
